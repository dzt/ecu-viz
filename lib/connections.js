...
const { ECU_GROUND, FUSEBOX } = require('./constants.js');

const connections = {};

connections.createFuseboxConnections = function(input, connectors) {
...
connections.createIgnitionConnections = function (cableSetup, connectors, cableTitle, input) {
    let connList = [];
    ...
    for (let i = 0; i < cableSetup.wirecount; i++) {
        if (i == 0) {
            ...
            for (let j = 0; j < (cableSetup.wirecount - 1); j++) {
                ...
                if (_.where(connectors.summary.auxiliary_outputs, { pn: auxDevice.pn }).length > 1) {
                    let duplicateIndexs = utils.findAllDuplicatesInListOfObjects(connectors.summary.auxiliary_outputs, 'pn')[0].indexes;
                    isMultiple = true;
                    for (let k = 0; k < duplicateIndexs.length; k++) {
...
connections.createInsertConnections = function(insert_ids, ecuID, usedAuxOutputs, usedDIs) {
