...
connections.createCANConnections = function(cableSetup, connectors, cableTitle, chassisCode) {
    ...
    for (let i = 0; i < cableSetup.wirecount; i++) {
        ...
        if (i == 0) { // 12v
            ...
            connList.push(connections.connectionHelper(keys, values));
        } else if (i == 1) { // ground, use closest ecu ground (shared)
            // console.log(_.findWhere(ecuPinout, {type: 'ground'}).pin)
            let keys = [ecuTitle, cableTitle, can_name]
            let values = [_.findWhere(ecuPinout, {type: 'ground'}).pin, yamlIndex, _.findWhere(can_pinout, { type: 'ground' }).pin]
            connList.push(connections.connectionHelper(keys, values));
        } else if (i == 2) { // can-h
            let keys = [ecuTitle, cableTitle, can_name]
