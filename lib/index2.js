...
class YamlGenerator {
    ...
    createConnectors () {
        ...
        let fusebox = connector.createFusebox(this.input);
        let buildArr = [
            connector.create(this.input.ecu, "ecu"), /* ECU connector */
            fusebox, /* Fusebox */
            connector.createECUGrounds(this.input.ecu), /* ECU ground connections */
            connector.createChassisConnections(this.input.chassis), /* Chassis connections */
            connector.createInsertConnections(this.input), /* Insert Connectors */
            connector.create(this.input.tps, "tps"), /* Throttle position sensor connection */
            connector.createInjectorConnections(this.input.injectors, this.input.engine), /* Injector connections */
            connector.createIgnitionConnections(this.input.ignition, this.input.engine), /* Ignition connections */
            connector.createTempConnections(this.input.clt, this.input.iat), /* Temperature sensors */
            ...
            connector.createCoilGrounds(this.input.engine), /* Ignition coil grounds */
    ...
    createCables () {
        let useChassisTacho = this.input.use_chassis_tacho;
        let ecuID = this.input.ecu;
        let cableObj = utils.builder([
            cables.createChassisConnections(this.summary.auxiliary_outputs, ecuID, useChassisTacho), /* Chassis connections */
            cables.createInjectors(this.summary.injector_outputs, ecuID), /* Injector connections */
            cables.createIgnitionConnections(this.summary.ignition_outputs, ecuID), /* Ignition connections */
            cables.createAnalogConnections(this.summary.analog_inputs, ecuID), /* Analog inputs */
            cables.createTempConnections(this.summary.temp_inputs, ecuID), /* Temperature sensors */
            cables.createAuxConnections(this.summary.auxiliary_outputs, ecuID), /* Auxiliary outputs */
            cables.createTriggerConnection(this.summary.trigger_options[0], ecuID), /* Trigger connections */
            ...
            (this.input.idle_valve != null) ? cables.createIdleValveConnection(_.findWhere(this.summary.auxiliary_outputs, { type: 'stepper_valve_options' }), ecuID) : null, /* Stepper Valve connector if applicable */
