...
cables.createIdleValveConnection = function(idleConnector, ecuID) {
    ...
    if ((typeof multipurpose_pins.stepper_valve == 'boolean') && (multipurpose_pins.stepper_valve == true)) {
        if (idleStepCount <= ecuStepperSpecificPins.length) {
            ...
            let ecuPinsToUse = ecuStepperSpecificPins.slice(ecuStepperSpecificPins.length - idleStepCount);
        ...
        console.log(multipurpose_pins.stepper_valve)
        console.log(`multipurpose_pins.stepper_valve.length: ${multipurpose_pins.stepper_valve.length}`)
        console.log(`idleStepCount: ${idleStepCount}`)

        let ecuPinsToUse = multipurpose_pins.stepper_valve.slice(multipurpose_pins.stepper_valve.length - idleStepCount).map((pinNumber) => {
            return _.findWhere(ecu.pinout, { pin: pinNumber });
        });
        console.log('ecuPinsToUse')
        console.log(ecuPinsToUse)
        for (let i = 0; i < ecuPinsToUse.length; i++) {
            let color = ecuPinsToUse[i].color;
            cableColors.push(utils.parseColor(color));
