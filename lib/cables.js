...
cables.createIgnitionConnections = function(ignition_assignments, ecu_id) {
    ...
    let count = pinout_filtered.length + 2; // default value (as if it were a 3 pin coil like a K20 style)
    if (pinout_filtered.length == 4) count += 1; // usually extra ground (i-e: LS coils)
    if (pinout_filtered.length == 5) count += 1; // usually 2 extra grounds (i-e: IGN-1A coils)
...
cables.createInsertCable = function(insert_ids) {
    let insertCables = []
    for (let i = 0; i < insert_ids.length; i++) {
        ...
        let insertCable = {
            ...
            gauge,
            show_equiv: true,
            colors: insert.colors.map(col => utils.parseColor(col))
        }
